...
<div class="page-banner">
  <div class="wrap">
    ...
    <div class="page-title-wrap">
      <h1 class="page-title">✏️ 공지 작성</h1>
      <p class="page-desc">공지사항을 작성합니다.</p>
    </div>
  </div>
</div>
...
<div class="board-content">
  <div class="wrap">
    <div class="write-card">
      ...
      <form class="write-form" action="<%= contextPath %>/notice/write" method="post"
        ...
        <div class="form-group">
          ...
          <div class="editor-toolbar">
            ...
            <button type="button" class="btn-img-attach" id="btnVideoLink" title="유튜브 동영상 링크 삽입">
              🎬 <span>동영상 링크</span>
            </button>
          ...
          <p class="img-paste-hint">💡 PC: 이미지 Ctrl+V · 모바일: 📷 카메라 / 🖼️ 갤러리 / 🎬 동영상 링크</p>
        </div>
        <div class="write-form-actions">
          <a href="<%= contextPath %>/notice/list" class="btn-cancel-write">취소</a>
            ...
            <tr>
              ...
              <td class="col-title">
                <a class="post-title-link"
                   href="<%= contextPath %>/notice/view/<%= lp.getPostNo() %>?page=<%= cp %>">
...
<script>
  ...
  function serializeNode(node) {
    ...
    if (node.nodeName === 'DIV' && node.classList && node.classList.contains('board-video-embed')) {
      ...
      const w = iframe && iframe.style.width  && !iframe.style.width.includes('%')  ? parseInt(iframe.style.width)  : 0;
      const h = iframe && iframe.style.height && !iframe.style.height.includes('%') ? parseInt(iframe.style.height) : 0;
  ...
  document.getElementById('btnVideoLink').addEventListener('click', function() {
    const url = prompt('동영상 URL을 입력하세요 (유튜브, 유튜브 Shorts)');
    if (!url) return;
    const video = parseVideoUrl(url.trim());
    if (video) { insertVideoAtCursor(video.type, video.id, editor); }
