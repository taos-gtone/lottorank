...
<div class="board-content">
  <div class="wrap">
    ...
    <div class="write-card">
      <div class="write-card-header">✏️ 글 수정</div>
      <form class="write-form" action="<%= contextPath %>/board/edit/<%= post.getPostNo() %>" method="post"
        ...
        <div class="write-form-actions">
          ...
          <button type="submit" class="btn-submit-write">수정 완료</button>
        </div>
      </form>
    </div>
    <% } else { %>
  ...
  <div class="wrap">
    <div class="board-card">
          ...
          <tbody>
            <% int rowNum = totalCount - (cp - 1) * 15;
               for (BoardPostVO lp : postList) {
                 boolean isCurrent = (lp.getPostNo() == editingPostNo); %>
            <tr<%= isCurrent ? " class=\"current-post-row\"" : "" %>>
              ...
              <td class="col-title">
                <a class="post-title-link"
                  ...
                  <% if (isCurrent) { %><span class="current-post-marker">✏️</span><% } %><%= org.springframework.web.util.HtmlUtils.htmlEscape(lp.getTitle()) %>
      ...
      <nav class="board-pagination">
        ...
        <% } else { %>
          <a href="<%= contextPath %>/board/edit/<%= editingPostNo %>?page=<%= cp - 1 %><%= filterParams %>" class="pg-btn">&#8249;</a>
        <% } %>

        <% if (startPage > 1) { %>
          <a href="<%= contextPath %>/board/edit/<%= editingPostNo %>?page=1<%= filterParams %>" class="pg-btn">1</a>
        ...
        <% for (int i = startPage; i <= endPage; i++) { %>
          ...
          <% } else { %>
            <a href="<%= contextPath %>/board/edit/<%= editingPostNo %>?page=<%= i %><%= filterParams %>" class="pg-btn"><%= i %></a>
          <% } %>
        <% } %>

        <% if (endPage < totalPages) { %>
          <% if (endPage < totalPages - 1) { %><span class="pg-ellipsis">···</span><% } %>
          <a href="<%= contextPath %>/board/edit/<%= editingPostNo %>?page=<%= totalPages %><%= filterParams %>" class="pg-btn"><%= totalPages %></a>
        <% } %>
