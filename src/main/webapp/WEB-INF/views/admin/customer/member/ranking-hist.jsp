...
<head>
  ...
  <title>회원 랭킹 이력 - 로또랭크 ADMIN</title>
  ...
  <style>
    ...
    .back-btn:hover { background: var(--g2); color: var(--g8); }
...
<%
  ...
  String dispNickname = memInfo != null && memInfo.getNickname() != null
          ? org.springframework.web.util.HtmlUtils.htmlEscape(memInfo.getNickname()) : "-";
  String dispUserId   = memInfo != null && memInfo.getUserId() != null
          ? org.springframework.web.util.HtmlUtils.htmlEscape(memInfo.getUserId()) : "-";

  String tabLabel  = isAll ? "전체기간 랭킹" : "최근 5주 랭킹";
  String backUrl   = "/lottorank/admin/customer/member/ranking?tab=" + tab;
  String pgBase    = "memberNo=" + memberNo + "&tab=" + tab;
%>
  ...
  <div class="page-hd">
    <div>
      <div class="page-hd-title">
        👤 회원 랭킹 이력
        <span class="mem-badge"><%= dispNickname %> (<%= dispUserId %>)</span>
      </div>
      <div class="page-hd-sub">
        <%= tabLabel %> · 전체 <strong><%= totalCount %></strong>회차 참여
      </div>
    </div>
    <a href="<%= backUrl %>" class="back-btn">← <%= tabLabel %>으로 돌아가기</a>
  </div>

  <!-- 탭 라벨 -->
  <div style="padding-bottom:0;">
    <span class="hist-tab-label"><%= isAll ? "📊" : "📅" %> <%= tabLabel %></span>
  </div>
  ...
  <div class="tbl-wrap">
    ...
    <table class="adm-table">
      ...
      <tbody>
        ...
        %>
        <tr><td class="empty" colspan="9">전체기간 랭킹 이력이 없습니다.</td></tr>
        <%
          } else {
        ...
        %>
        <tr>
          <td><span class="round-badge">제 <%= r.getRoundNo() %>회</span></td>
    ...
    <table class="adm-table">
      ...
      <tbody>
        ...
        %>
        <tr><td class="empty" colspan="10">최근5주 랭킹 이력이 없습니다.</td></tr>
        <%
          } else {
